...
class Actions
{
	...
	public function init()
	{
		...
		add_filter('sgpbLoadablePopups', array($this, 'filterLoadablePopupsForWpml'), 100, 1);
	...
	/**
	 * Final WPML filter: drop popups whose language does not match (covers paths that skip other conditions).
	 *
	 * @param array $popups SGPopup instances.
	 * @return array
	 */
	public function filterLoadablePopupsForWpml($popups)
	{
		if (!is_array($popups) || !PopupChecker::isWpmlActive()) {
			return $popups;
		}
		$filtered = array();
		foreach ($popups as $popup) {
			if (!is_object($popup) || !method_exists($popup, 'getId') || !method_exists($popup, 'getOptions')) {
				$filtered[] = $popup;
				continue;
			}
			...
			if (PopupChecker::wpmlAllowsPopupFrontendLoad($popupId, $options, $args)) {
				$filtered[] = $popup;
			}
		}
